ALTER TABLE public.slots_stock
  ADD COLUMN IF NOT EXISTS profile_password varchar(255) NULL;

DROP FUNCTION IF EXISTS public.allocate_slot_for_order(uuid);

CREATE OR REPLACE FUNCTION public.allocate_slot_for_order(p_order_id uuid)
 RETURNS TABLE(slot_id uuid, account_email character varying, account_password character varying, slot_number integer, profile_name character varying, profile_password character varying, application_name character varying, remaining_stock integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
